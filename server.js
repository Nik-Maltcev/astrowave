...
app.post('/api/neuro-horoscope', async (req, res) => {
  if (!process.env.DEEPSEEK_API_KEY) {
    ...
    return res.json({
      success: true,
      data: {
        theme: "Год Квантового Скачка",
        general: "2026 год станет для тебя переломным моментом. Транзит Сатурна активирует твой сектор инноваций, требуя отказа от старых паттернов. Нейросеть видит высокую вероятность смены парадигмы в марте и ноябре. Технологические тренды будут играть на руку твоей природной интуиции, позволяя видеть возможности там, где другие видят хаос. Это год, когда ты наконец сможешь монетизировать свои уникальные таланты.",
        career: "В карьере ожидается смещение фокуса с тяжелого труда на умные алгоритмы. Возможно неожиданное предложение в сфере EdTech или цифровых активов. Твой доход вырастет, если ты перестанешь бояться делегировать рутину искусственному интеллекту. Ключевые месяцы: Май, Сентябрь.",
        love: "В сфере отношений цифровая прозрачность станет новой искренностью. Для одиноких высок шанс встречи через новые платформы или в сообществах по интересам (VR/AR). В существующих парах потребуется 'обновление прошивки' — честный разговор о совместном будущем в эпоху перемен.",
        advice: "Не бойся переписать свой код: то, что работало в 2025, устареет к лету."
      }
    });
  }
